#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @author : XuZelin Updated from R XIE
...
isJFWT = True  # 配置是否是金风风机(True),明阳风机(False)
offline = False  # 区别上线离线
...
# dex(['time', 'windSpeed', 'windDirection', 'power', 'rotorSpeed',
#        'nacelleVib_X', 'nacelleVib_Y', 'pitch_1', 'pitch_2', 'pitch_3',
#        'tempreature', 'cabin_tempreature', 'torque_demand', 'torque_measure',
#        'turbineName', 'temperature', 'yawError'],
...
wtstatus_n = [0] if isJFWT else [0]  # 正常状态，JF 5  明阳  14  ,原来37,38
has_wtstatus = "False"  # 'True' #TODO 明阳W010需要修改
cut_speed = 4 #TODO 新增2个参数
rated_speed = 8
rotor_radians =65.0
...
def smooth_data(data, var, yaw, var_u=5, var_l=0, yaw_w=0.2, bin_minnum_upper=50, bin_minnum_lower=10, std_limit=1,
                upper=90, lower=10, range_limit=1.5, method='iForest', contamination=0.01):
    '''
    ...
    if method == 'iForest':
        ...
        if len(data) > 0:
            ...
            # ########
            # data['label'] =1 ####不删除，For test
            # #######
            new_data = data.loc[data['label'] == 1, :]  # 将异常的数据去除，降噪
#############数据剔除Plot
            plt.scatter(data[data['label'] == 1][yaw],data[data['label'] == 1][var] , s=0.5)
            plt.scatter(data[data['label'] != 1][yaw], data[data['label'] != 1][var], s=1)
            plt.show()
            ...
            if len(sdata) >= bin_minnum_upper:
                if method == 'A':
                    ...
                    while (window_width >= range_limit) & (len(sdata) > bin_minnum_lower):
                        if sdata.loc[sdata['distance'] == max(sdata['distance']), var].values[0] > min( sdata.loc[sdata['distance'] >= np.median(sdata['distance']), var].values):
...
def yaw_misalignment(data, ws=ws, gp=gp, yaw=yaw, time=ts, yaw_left_edge=-30, yaw_right_edge=30, min_data_need=1000,
                     smooth_mode='True', contamination=0.01, method='iForest'):  #method='iForest'
    """
    对风偏航不正识别模型
    :param data: 输入数据，dataframe
    :param ws: 风速变量名
    ...
    start_time = str(min(data[time]))
    end_time = str(max(data[time]))

    resampled = resample_bytime_mean(data, 1, time)
    raw_data = dict()
    resampled = resampled.dropna()
    raw_data['time'] = list(map(str, pd.to_datetime(resampled[time].values.tolist())))
    raw_data['yaw'] = list(map(lambda x: float(Decimal(x).quantize(Decimal('0.0000'))), resampled[yaw].values.tolist()))
    raw_data['windspeed'] = list( map(lambda x: float(Decimal(x).quantize(Decimal('0.0000'))), resampled[ws].values.tolist()))
    raw_data['power'] = list(  map(lambda x: float(Decimal(x).quantize(Decimal('0.0000'))), resampled[gp].values.tolist()))

    # data = data.loc[(data[ws] > cut_speed) & (data[ws] <= rated_speed) & (data[yaw] > yaw_left_edge) & ( data[yaw] < yaw_right_edge) & (data[gp] > 0), :]
    data = data.loc[(data[ws] > 3) & (data[ws] <= 9) & (data[yaw] > -20) & ( data[yaw] < 20) & (data[gp] > 20), :]
    data = data.loc[(data[ba] < 0.1) & (data[ba] > -0.9) , :]



    #######Todo:此处筛选可换为叶尖速比 Jimmy-20211122

    # data['gpindex'] = data[gp].values / (data[ws].values ** 3)
    # resampled['gpindex'] = resampled[gp].values / (resampled[ws].values ** 3)

    data['gpindex'] = data[gp].values*1000 / (0.5*1.225*math.pi * rotor_radians**2 * data[ws].values ** 3)
    resampled['gpindex'] = resampled[gp].values*1000 / (0.5*1.225*math.pi * rotor_radians**2 *resampled[ws].values ** 3)



    # plt.subplot(3,2,1)
    # plt.scatter(data[ws], data['lambda'], s=0.5, label='windSpeed-lambda')
    # plt.grid()
    # plt.legend()
    # plt.subplot(3, 2, 2)
    # plt.scatter(data[ws], data['gpindex'], s=0.5,label='windSpeed-Cp')
    # plt.legend()
    # plt.subplot(3, 2, 3)
    # plt.scatter(data[ws], data[gp], s=0.5,label='windSpeed-power')
    # plt.legend()
    # plt.subplot(3, 2, 4)
    # plt.scatter(data[ws], data[rs], s=0.5,label='windSpeed-rotorSpeed')
    # plt.legend()
    # plt.subplot(3, 2, 5)
    # plt.scatter(data[rs], data[gp], s=0.5,label='rotorSpeed-power')
    # plt.legend()
    # plt.subplot(3, 2, 6)
    # plt.scatter(data[gp], data[ba], s=0.5,label='power-pitchAngle')
    # plt.legend()
    # plt.show()
    ...
    if len(data) > min_data_need:
        if smooth_mode == 'True':
            data = smooth_data(data, 'gpindex', yaw, var_u=20, var_l=0, yaw_w=1, bin_minnum_upper=50,
                               bin_minnum_lower=5,
                               std_limit=1,
        ...
        if len(data) > 0:
            ...
            # ###用叶尖速比分bin
            # wsbreaks = np.arange(8, 13, 0.5)  # 换成lambda
            # data['ws_group'] = pd.cut(data['lambda'], wsbreaks)

            wslist = pd.unique(data['ws_group'].values)

            curve_collector, maxwdir_collector, maxcp_collector = dict(), dict(), dict()
            yawData2 = np.arange(min(data[yaw]), max(data[yaw]), 1)

            for wsg in wslist:  # 风速bin
                sdata = data.loc[data['ws_group'] == wsg, :]  # 没有值 ==None  风速分bin
                plt.scatter(sdata[yaw], sdata['gpindex'], s=0.5)
                ...
                if len(sdata) > 0:
                    ...
                    for yaw_value in yawData2:
                        ...
                        if len(sdata2) > 0:
                            # sdata3 = sdata2.loc[sdata2['gpindex'] >= np.percentile(sdata2['gpindex'], 90)]

                            sdata3 = sdata2[(sdata2['gpindex'] >= np.percentile(sdata2['gpindex'], 25)) & (  sdata2['gpindex'] <= np.percentile(sdata2['gpindex'], 75))]
                            # sdata3 = sdata2[(sdata2['gpindex'] >= np.percentile(sdata2['gpindex'], 90)) & (
                            #             sdata2['gpindex'] <= np.percentile(sdata2['gpindex'], 99))]
                            # 大于99分位数的风能利用系数中位数， 改成了25-75分位 Jimmy-20211123

                            max_gpindex = np.median(sdata3['gpindex'].values)
                    ...
                    # ##################### plot每个区间的特征点
                    # plt.plot(yaw_gp_d['yaw'], yaw_gp_d['gpindex'], label=str(wsg), linewidth=2, marker="o")

                    # if (len(yaw_collector) > 0) & (np.std( [maxgp for maxgp in max_gpindex_collector if (maxgp != -1) & (~np.isnan(maxgp))]) >= 0.2):######为什么要标准偏差大于0.2
                    if (len(yaw_collector) > 0):
                        lowess = sm.nonparametric.lowess
                        model = pd.DataFrame(lowess(np.array(yaw_gp_d['gpindex'].values),
                        ...
                        # ################Plot 每条拟合的线
                        # plt.plot(itpld['Wdir'], itpld['PowerIndex'])
                        ...
                        maxcp_collector[wsg] = maxpred

            maxwdir_list = list(maxwdir_collector.values())
            key_list = list(maxwdir_collector.keys())

            # for key in key_list:
            #     print (key, maxwdir_collector[key], maxcp_collector[key])
            # plt.ylim(0,1.5)
            # plt.legend()
            # plt.show()
            ...
            if len(maxwdir_list) > 0:  # 评估可靠性等级，并把数据赋值给analysis_data
                ...
                plt.ylabel('Cp_特征值')
                plt.legend()
                plt.title(data[wt_id][0]+'_'+ start_time[0:10] )

                # plt.show()
                savePath = '../Result/yaw_misalignment/{}/'.format(data[wt_id][0])
                ...
                if len(maxwdir_list) >= 2:
                    ...
                    if (
                            (np.std(maxwdir_list) < 1) & (len(maxwdir_list) > 0.3 * len(wsbreaks)) &
                        ...
                        if (max(itpld.loc[itpld['PowerIndex'] > min(itpld['PowerIndex'].sort_values(ascending=False)[ 0: max(int(len(itpld) * 0.1), 3)]), 'Wdir'])
                            ...
                            if (max(itpld.loc[itpld['PowerIndex'] > min( itpld['PowerIndex'].sort_values(ascending=False)[   0:max(int(len(itpld) * 0.1), 3)]), 'PowerIndex'])
                                ...
                                maxwdir = float(Decimal( max(itpld.loc[itpld['PowerIndex'] == max(itpld['PowerIndex']), 'Wdir'])).quantize( Decimal('0.0000')))
                            ...
                            (
                            ((np.std(maxwdir_list) >= 1) & (np.std(maxwdir_list) < 3) & ( len(maxwdir_list) > 0.1 * len(wsbreaks)))
...
def yaw_misalignment_wrapper(data, ws, gp, yaw, time, yaw_left_edge, yaw_right_edge, min_data_need, ba,
    ...
    # plt.scatter(data[gp], data[ba], s=0.5)
    # plt.show()

    if len(data) > 0:
        data = data.dropna(subset=[ws, gp, yaw, time, gs])
        data['lambda']= data[gs] * math.pi / 30 * rotor_radians / data[ws]
    ...
    # print(result['start_time'], result['max_wdir'], result['distance'], result['alarm'], result['status_code'], result["rank"])

    if offline:
        return result  # 离线
    else:
        return (result["start_time"], result["end_time"], result["raw_data"], result["analysis_data"],
                result["status_code"], result["max_wdir"], result["alarm"], result["distance"], result["rank"])


def data_preprocess(data):
    columns = [ts, wt_id, ws, yaw, ba, gp, wt_status, gs] if has_wtstatus == "True" else [ts, wt_id, ws, yaw, ba, gp, gs]
    data = data[columns]
    if has_wtstatus == "True":
        data[wt_status].fillna(method="ffill", inplace=True)
        data[wt_status].fillna(method="bfill", inplace=True)
...
def result_plot(result, res_index):
    ...
    if result['analysis_data'] and result['analysis_data']["online_x"]:
        ...
        ax3.set_title(
            f"偏差角度：{result['max_wdir']},预警等级：{result['alarm']},健康值：{result['distance']},结果可靠等级：{result['rank']}")
...
if __name__ == '__main__':
    ...
    # with open("../YawMisalignment-85/YawMisalignment-85-data.json","r") as f:
    #     data  = json.load(f)
    # res = pd.DataFrame(np.array(data),columns=["info"])
    # for col in ["wind_speed", "active_power", "wind_direction", "pitch_angle_1" ,"t" , 'main_status' ]:
    #     res[col] = res["info"].apply(lambda x:x.get(col,None))

    # res = pd.read_json("../YawMisalignment-85/YawMisalignment-85-data.json")

    # 处理成标准格式的文件夹，每个机组一个csv,文件名代表机组名称
    pre_Dir = 'C:\\Project\\03_AeroImprovement\\05_华润风机SCADA数据分析_202111\\华润风机数据\\数据\\Prep_01\\01'
    report_Dir = 'C:\\Project\\03_AeroImprovement\\05_华润风机SCADA数据分析_202111\\Report_01\\01\\'

    neg_turbine_number=6

    list_df_WTs = []
    list_df_WTs = reLoad_ScadaData(pre_Dir, neg_number=neg_turbine_number)  # 载入处理成标准格式的数据， 第二个数字表示载入数量=总数-negNumber 不删减

    # print ('done')
    # plt.scatter(res.index, res[ba], s=0.5)
    # plt.show()
    start_time = '2021-1-1 00:00:00'
    for i in range(len(list_df_WTs)):
        for n in range(10):
            time_range = pd.date_range(start_time, periods=2, freq='300D')
            st = time_range[0]
            et = time_range[1]
            list_df_WTs[i]['main_status'] = 5
            sample_data = list_df_WTs[i][(list_df_WTs[i][ts] >= st) & (list_df_WTs[i][ts] < et)]

            sample_data= resample_bytime_mean(sample_data, 1, ts)
            sample_data = sample_data.dropna()
            sample_data.reset_index(inplace=True)
            sample_data['gs_square']= (sample_data[gs] * math.pi / 30 * 154.5) ** 2



            # plt.scatter(sample_data['torque_demand'],sample_data['torque_measure'], s=0.5)

            sample_data['Tg_cal']= sample_data[gp]*1000/(sample_data[gs] * math.pi / 30 * 154.5)

            sample_data['Kopt'] =sample_data['Tg_cal']/sample_data['gs_square']
            # plt.scatter(sample_data['T_cal'], sample_data['torque_measure'], s=0.5)

            # plt.scatter(sample_data['Tg_cal'], sample_data['Kopt'], s=0.5)
            # plt.xlim(0,20000)
            # plt.ylim(0, 1)
            ...
            result={}
            result["start_time"], result["end_time"], result["raw_data"], result["analysis_data"],  result["status_code"], result["max_wdir"], result["alarm"], result["distance"], result["rank"]=  yaw_misalignment_main(sample_data)
            print(to_alarm(result["max_wdir"], alarm_threshold), 'max_wdir= ', result["max_wdir"], ' status_code= ', result["status_code"], 'rank=', result["rank"], start_time)
            start_time = et
